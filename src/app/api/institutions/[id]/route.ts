 ...
 * @param params.id ID de la institución a actualizar (extraído de la URL)
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!params.id || !validateInstitution(body)) {
      return NextResponse.json({ error: "Datos inválidos." }, { status: 400 });
    }

    const { data, error } = await supabase.from("institutions").update(body).eq("id", params.id).select();
 ...
 * @param params.id ID de la institución a eliminar (extraído de la URL)
